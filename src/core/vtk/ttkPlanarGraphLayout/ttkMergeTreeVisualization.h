...
public:
  ...
  void makeTreesOutput(std::vector<FTMTree_MT *> &trees,
    ...
    vtkNew<vtkIntArray> trueDownNodeId{};
    trueDownNodeId->SetName("trueDownNodeId");
          ...
          if(!trees[i]->isRoot(node) or isPersistenceDiagram) {
            ...
            for(int toAddT = 0; toAddT < toAdd; ++toAddT) {
              ...
              if(not isPersistenceDiagram) {
                ...
                trueDownNodeId->InsertNextTuple1(node);
              }
    ...
    if(not isPersistenceDiagram) {
      ...
      vtkArcs->GetCellData()->AddArray(trueDownNodeId);
    }
    if(clusteringOutput and ShiftMode != 1) {
    ...
    // Add cell vertices
    // ttkUtils::CellVertexFromPoints(vtkOutputNode, points);
