  ...
  for(int c = 0; c < k_; c++) {
    for(size_t i = 0; i < clustering_[c].size(); i++) {
      ...
      if(original_dos[0]) {
        ...
        // std::cout<<"sizes "<<current_bidder_diagrams_min_[diagram_id].size()<<
        //   " "<<current_bidder_ids_min_[diagram_id].size()<<std::endl;
      ...
      if(original_dos[1]) {
        // std::cout<<"saddles match"<<std::endl;
        // 1. Invert the current_bidder_ids_ vector
        std::vector<int> new_to_old_id(
    ...
    if(!clustering_[c].empty()) {
      ...
      if(do_min_) {
        ...
        barycenter_computer_min_[c].runMatching(
          ...
          // std::cout<<"MIN MATCHINGS"<<std::endl;
        // for(size_t ii = 0; ii < all_matchings.size(); ii++) {
          // std::cout<<"matchings for diag "<<ii<<std::endl;
          // for(size_t jj = 0; jj < all_matchings[ii].size(); jj++) {
          // auto& m = all_matchings[ii][jj];
      ...
      if(do_sad_) {
        ...
        barycenter_computer_sad_[c].runMatching(
          ...
          // std::cout<<"SADDLE MATCHINGS"<<std::endl;
        // for(size_t ii = 0; ii < all_matchings.size(); ii++) {
          // std::cout<<"matchings for diag "<<ii<<std::endl;
          // for(size_t jj = 0; jj < all_matchings[ii].size(); jj++) {
          // auto& m = all_matchings[ii][jj];
