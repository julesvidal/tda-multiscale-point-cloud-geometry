...
namespace ttk {
  ...
  public:
    ...
    void sortPersistenceDiagramByDeath(std::vector<PersistencePair> &diagram,
                                const SimplexId *const offsets) const;
    ...
    int executeProgressiveTopology(std::vector<PersistencePair> &CTDiagram,
                                   ...
                                   const triangulationType *triangulation);
    template <class triangulationType>
    int executeUF(std::vector<PersistencePair> &CTDiagram,
                                   const SimplexId *inputOffsets,
                                   const triangulationType *triangulation);
    template <typename scalarType, class triangulationType>
    ...
    // const std::vector<SimplexId>& getCCSizes() const{
    //   return dms_.getCCSizes();
    // }
    // const std::vector<SimplexId>& getCCParents() const{
    //   return dms_.getCCParents();
    // }
...
int ttk::PersistenceDiagram::execute(std::vector<PersistencePair> &CTDiagram,
  ...
  switch(BackEnd) {
    ...
    case BACKEND::FTM:
      ...
      break;
    case BACKEND::UF:
      executeUF(CTDiagram, inputOffsets, triangulation);
      break;
    default:
...
//   std::vector<SimplexId> goodIndex(CTDiagram.size());
// #ifdef TTK_ENABLE_OPENMP
// #pragma omp parallel for num_threads(threadNumber_)
// #endif // TTK_ENABLE_OPENMP
//   for(size_t i = 0; i < CTDiagram.size(); ++i) {
//     goodIndex[CTDiagram[i].birth.id] = i;
//   }
...
// #endif // TTK_ENABLE_OPENMP
//   for(size_t i = 0; i < CTDiagram.size(); ++i) {
//     // auto& cid = CTDiagram[i].cc_parent;
//     CTDiagram[i].cc_parent = goodIndex[CTDiagram[i].cc_parent];
//   }
//   std::cout<<"done reverse index thingy"<<std::endl;
...
int ttk::PersistenceDiagram::executeDiscreteMorseSandwich(
  ...
  // const std::vector<SimplexId>& cc_size = dms_.getCCSizes();
  // const std::vector<SimplexId>& cc_parent = dms_.getCCParents();
...
#pragma omp parallel for num_threads(threadNumber_)
  ...
  for(size_t i = 0; i < dms_pairs.size(); ++i) {
    ...
    const auto isFinite = (p.death >= 0);
    const auto death = isFinite ? p.death : globmax;
    // const auto size = cc_size[p.birth];
    // const auto parent = cc_parent[p.birth];
    // std::cout<<"dms pair "<<i<<" of birth "<<p.birth<<", size "<<size<<std::endl;

    if(p.type == 0) {
      // std::cout<<" adding pair "<<p.birth<<"-"<<p.death<<" of size "<<size<<" and parent "<<parent<<std::endl;
      const auto dtype = (isFinite && dim > 1) ? CriticalType::Saddle1
                                               : CriticalType::Local_maximum;
      CTDiagram[i] = PersistencePair{
        CriticalVertex{p.birth, CriticalType::Local_minimum, {}, {}},
        CriticalVertex{death, dtype, {}, {}}, p.type, isFinite, 0, 0};
    } else if(p.type == 1) {
      const auto btype
        = (dim == 3) ? CriticalType::Saddle1 : CriticalType::Saddle2;
...
template <class triangulationType>
int ttk::PersistenceDiagram::executeUF(
  std::vector<PersistencePair> &diagram,
  const SimplexId *inputOffsets,
  const triangulationType *triangulation) {

  auto nbPoints = triangulation->getNumberOfVertices();
  std::cout<<nbPoints<<" points"<<std::endl;
  // auto edges =  *(triangulation->getEdges());
  auto nedges = triangulation->getNumberOfCells();
  std::cout<<nedges<<" edges"<<std::endl;


  std::cout<<"get edges"<<std::endl;
  std::vector<std::array<SimplexId,2>> edges(nedges);
  for(SimplexId iEdge = 0; iEdge<nedges; iEdge++){
    SimplexId v0, v1;
    triangulation->getCellVertex(iEdge,0,v0);
    triangulation->getCellVertex(iEdge,1,v1);
    if(inputOffsets[v0]>inputOffsets[v1]){
      std::swap(v0,v1);
    }
    edges[iEdge] = {v0,v1};
  }
  std::vector<SimplexId> edgeOffsets{};
  std::cout<<"edge offsets"<<std::endl;

  for(const auto& e : edges){
    auto v0 = e[0];
    auto v1 = e[1];
    edgeOffsets.push_back(std::max(inputOffsets[v0], inputOffsets[v1]));
  }
  std::vector<SimplexId> edgeOrder(nedges);
  ...
  auto cmp =
    [=](
      const SimplexId &e0,
      const SimplexId &e1) {
      if(edgeOffsets[e0]!=edgeOffsets[e1]){
        return edgeOffsets[e0]<edgeOffsets[e1];
      }else{
        auto u0 = edges[e0][0];
        auto v0 = edges[e0][1];
        auto u1 = edges[e1][0];
        auto v1 = edges[e1][1];
        return std::min(inputOffsets[u0],inputOffsets[v0]) < std::min(inputOffsets[u1],inputOffsets[v1]);
      }
    };


    TTK_PSORT(threadNumber_, edgeOrder.begin(), edgeOrder.end(), cmp);
    std::cout<<"sorted"<<std::endl;


    std::vector<SimplexId> reps(nbPoints);
    std::iota(reps.begin(), reps.end(), 0);
      ...
      const auto merge = [&](SimplexId cc0, SimplexId cc1) -> SimplexId {
       ...
       if(r0!=r1){
        if(inputOffsets[r0]<inputOffsets[r1]){
          std::swap(r0, r1);
        }
      ...
      const auto addPair = [&](SimplexId birthId, SimplexId edgeId, bool isFinite){
          diagram.emplace_back(PersistencePair{
              CriticalVertex{birthId, CriticalType::Local_minimum, {} ,{}},
              CriticalVertex{edgeId, CriticalType::Saddle1, {},{}},
              0, isFinite });
      };
      ...
      for(auto& i : edgeOrder){
        std::cout<<"offset "<<edgeOffsets[i]<<std::endl;
        const auto& e = edges[i];

        SimplexId c0 = e[0];
        SimplexId c1 = e[1];
        SimplexId saddle = inputOffsets[c0]>inputOffsets[c1] ? c0 : c1;

        SimplexId dyingCC = merge(c0,c1);
        if(dyingCC != -1){
         addPair(dyingCC, saddle, true);
        }
      }
      ...
      if(toMerge.size()>0){
        const auto& last_edge = *edges.end();
        SimplexId highest_vertex = inputOffsets[last_edge[0]]<inputOffsets[last_edge[1]] ? last_edge[1] : last_edge[0];
        for(auto& cc : toMerge){
          addPair(cc, highest_vertex, true);
          std::cout<<" add last pair "<<cc<<"-"<<highest_vertex<<" offsets "<<inputOffsets[cc]<<" "<< inputOffsets[highest_vertex]<<std::endl;
        }
      }
