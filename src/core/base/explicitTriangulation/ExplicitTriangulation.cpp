...
int ExplicitTriangulation::preconditionEdgesInternal() {
  ...
  std::cout<<"PRECONDICTION EDGES "<<getDimensionality()<<std::endl;
  if(this->cellArray_ == nullptr || this->vertexNumber_ == 0) {
#ifdef TTK_ENABLE_MPI
    if(!(ttk::isRunningWithMPI()))
#endif
  ...
  if(edgeList_.empty()) {
    ...
    if(getDimensionality() == 1) {
      ...
      std::cout<<"build skel, edgelist "<<&edgeList_<<std::endl;
    ...
    std::cout<<"edge list is not empty"<<std::endl;
    std::cout<<"size "<<edgeList_.size()<<std::endl;
    std::cout<<&edgeList_<<std::endl;
