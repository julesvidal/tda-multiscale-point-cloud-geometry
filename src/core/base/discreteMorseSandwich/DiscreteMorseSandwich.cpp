...
void ttk::DiscreteMorseSandwich::tripletsToPersistencePairs(
  ...
  // if(pairDim==0){
  //   std::cout<<"EXTREMA ARRAY SIZE "<<pairedExtrema.size()<<std::endl;
  //   cc_sizes_.resize(pairedExtrema.size(),1);
  //   std::fill(cc_sizes_.begin(), cc_sizes_.end(), 1);
  //   merges_.resize(pairedExtrema.size());
  //   std::iota(merges_.begin(),merges_.end(),0);
  // }
  ...
  for(const auto &t : triplets) {
    ...
    if(r1 != r2) {
      ...
      // if(cc_sizes_[r1]>cc_sizes_[r2]){
      //   std::swap(r1, r2);
      // }
      if(!pairedExtrema[r1]) {
        addPair(sv, r1);
        // if(pairDim==0){
        //   cc_sizes_[r2]+=cc_sizes_[r1];
        //   merges_[r1]=r2;
        // }
        reps[t[1]] = r2;
        reps[r1] = r2;
  ...
  // printMsg("Found pairs ",1,tm.getElapsedTime(),threadNumber_);
}
